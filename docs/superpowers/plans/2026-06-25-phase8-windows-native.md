...
- [x] 实现连接后的单条 SSH 命令执行：复用 SSH.NET adapter 和连接状态机，命令输入会 trim，空命令被拒绝，stdout/stderr 写入工作台输出区，并保留会话内最近命令；选择最近命令只会填回输入框，不会立即执行远端命令；运行中命令可从命令区取消，取消的命令不会进入最近命令。
...
- [x] 单条命令执行入口：工作台 Overview 提供命令输入框、Run/Cancel 按钮和最近命令列表，连接成功且命令非空时可执行，输出复用命令输出区。
