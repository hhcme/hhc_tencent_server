...
- [x] 实现失败、取消和中断传输任务的历史重试入口；rsync 路径可利用 `--partial` 保留的部分文件继续传输，SFTP fallback 负责普通上传/下载，真正断点续传留到正式 SFTP 队列阶段。
