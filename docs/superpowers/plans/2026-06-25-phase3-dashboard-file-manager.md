...
- SSH 指标必须标明来源：SSH。
- 正式 SFTP、字节流实时百分比、并发传输和真正断点续传未落地前，文件管理器只能宣称支持 bootstrap 排队批量传输；当前可持久化 pending/running 任务并在重新进入工作台时把遗留未完成任务恢复为 interrupted。
...
- [ ] 实现正式 SFTP/字节流实时进度、并发传输和真正可恢复/断点续传队列。
- [x] 实现重命名。
- [x] 实现权限查看基础展示。
