...
- Dashboard 当前为 Phase 3 bootstrap：指标通过 SSH 即时采集，支持手动刷新和自动刷新；已关联腾讯云 CVM 时会通过 Cloud Monitor `GetMonitorData` 拉取 Cloud CPU 指标，已关联阿里云 ECS 时会通过 CMS `DescribeMetricList` 拉取 Cloud CPU 指标，已关联华为云 ECS 时会通过 CES `metric-data` 拉取 Cloud CPU 指标，并标记来源为 Cloud API；单项可选指标失败会以 warning 降级展示；成功刷新会写入 `dashboard_snapshots` 缓存表并在重新进入工作台时恢复最近快照。更多云监控指标仍待扩展。
...
- 云账号当前已实现本地元数据、云实例关联表、云盘/快照/计费状态表、Keychain 云凭据命名空间、Tencent / Alibaba / Huawei Cloud adapter、云实例同步服务、腾讯云云盘/快照/计费状态同步服务、阿里云云盘/快照/安全组同步、快照创建/删除、云盘挂载/卸载、实例启动/停止/重启、安全组单条规则新增/删除和 CPU 云监控查询、华为云 EVS 云盘/快照/VPC 安全组同步、快照创建/删除、云盘挂载/卸载、实例启动/停止/重启、安全组单条规则创建/删除和 CPU 云监控查询、腾讯云快照创建/删除操作、腾讯云云盘挂载/卸载操作、腾讯云实例启动/停止/重启操作、腾讯云安全组单条规则新增/删除、统一资源搜索、三家云基础导入 UI、云资源中心 UI、已关联腾讯云 CVM / 阿里云 ECS / 华为云 ECS 的 CPU 云监控查询和账号地域级安全组查询；阿里云目前覆盖地域、ECS 实例、实例启动/停止/重启、云盘、云盘挂载/卸载、快照、安全组、安全组单条规则新增/删除、快照创建/删除、实例派生计费状态和 CMS CPU 指标，华为云目前覆盖项目发现、ECS 实例、实例启动/停止/重启、EVS 云盘、EVS 云盘挂载/卸载、EVS 快照、EVS 快照创建/删除、安全组、安全组单条规则创建/删除、实例派生计费状态和 CES CPU 指标，更多跨云危险云写操作仍在后续任务中。真实多云手动验收、真实腾讯云快照/云盘/实例电源/安全组写操作验收和更多跨云危险云写操作仍在后续任务中。
