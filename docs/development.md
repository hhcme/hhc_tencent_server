...
- SQLite repository：服务器 CRUD、trusted host key 级联删除、命令历史、操作日志、远程变更审计日志、云账号元数据、云实例关联、私有仓库实例和备份历史。
