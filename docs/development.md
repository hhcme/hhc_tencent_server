...
- ServerWorkspaceViewModel：连接状态、主机指纹确认、smoke test、单条命令执行与取消、本次会话输出历史、stdout/stderr 分开展示、失败摘要、持久化命令元数据历史、历史命令重跑、Dashboard 手动/自动刷新、远程目录浏览、排队批量上传/下载、当前传输取消、待传队列暂停/恢复/排序/清空、传输任务状态记录、重命名、chmod 权限修改、可恢复移入回收目录、轻量文本编辑、腾讯云安全组查看和单条规则变更、systemd 服务管理、Cron 管理、Nginx 配置管理、Firewall 状态流、Environment 文件管理和 Audit 日志查看。
