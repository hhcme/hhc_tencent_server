...
- 云账号当前已实现本地元数据、云实例关联表、云盘/快照/计费状态表、Keychain 云凭据命名空间、Tencent / Alibaba / Huawei Cloud adapter、云实例同步服务、腾讯云云盘/快照/计费状态同步服务、统一资源搜索、三家云基础导入 UI、云资源中心 UI、已关联 CVM 的 CPU 云监控查询和账号地域级安全组只读查询；阿里云/华为云目前只覆盖地域或项目发现、ECS 实例发现和实例派生计费状态，云盘/快照/安全组等高级资源仍未接入。危险云写操作和真实多云手动验收仍在后续任务中。
