...
- NginxConfigManager：通过 SSH 动态探测 Nginx 配置路径，优先读取 `nginx -V` 的 `--conf-path` / `--prefix`，并兼容 `/etc/nginx`、`/usr/local/nginx/conf`、`/opt/nginx/conf` 等常见目录；支持配置文件列表、UTF-8 配置读取、`nginx -t` 和确认后 reload。
...
- Nginx 当前为 Phase 4 bootstrap：配置路径通过 `nginx -V` 动态探测，已覆盖 `/etc/nginx` 和 `/www/server/nginx/conf` 这类非标准安装路径；配置文件可只读浏览，`nginx -t` 失败会阻止 reload，reload 需要 UI 确认并写入 `remote_change_logs` 审计表。真实服务器已完成 `nginx -t` 和配置目录只读验证；配置编辑、保存前备份、测试失败自动恢复备份仍在后续任务。
