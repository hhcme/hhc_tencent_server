...
- Private Registries 当前为 Phase 6 bootstrap：已建立 Verdaccio 安装草稿、配置校验、固定版本策略、安装前 SSH preflight 检查、可读检查报告、基础 `config.yaml` 和 systemd service 模板；preflight 会检查 Node.js、npm/pnpm/yarn、systemd、端口占用、安装/data 目录父级可写性和磁盘空间。当前尚未执行远程安装、创建目录、写配置、启动服务、备份恢复、Nginx proxy 或 Dart/Flutter pub 方案验证。
