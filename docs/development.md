...
- 云账号当前已实现本地元数据、云实例关联表、云盘/快照/计费状态表、Keychain 云凭据命名空间、Tencent / Alibaba / Huawei Cloud adapter、云实例同步服务、腾讯云云盘/快照/计费状态同步服务、阿里云云盘只读同步、腾讯云快照创建/删除操作、腾讯云云盘挂载/卸载操作、腾讯云实例启动/停止/重启操作、腾讯云安全组单条规则新增/删除、统一资源搜索、三家云基础导入 UI、云资源中心 UI、已关联 CVM 的 CPU 云监控查询和账号地域级安全组查询；阿里云目前覆盖地域、ECS 实例、云盘和实例派生计费状态，华为云目前覆盖项目发现、ECS 实例发现和实例派生计费状态，快照/安全组等更多跨云高级资源仍未接入。真实多云手动验收、真实腾讯云快照/云盘/实例电源/安全组写操作验收和更多跨云危险云写操作仍在后续任务中。
...
- AlibabaCloudAdapter 已接入阿里云 OpenAPI V3 header 签名流程，并实现 ECS `DescribeRegions`、`DescribeInstances`、`DescribeDisks` 只读查询；HuaweiCloudAdapter 已接入华为云 AK/SK 签名流程，并实现 IAM project 发现和 ECS `cloudservers/detail` 只读查询。默认测试使用 mock transport，不提交真实 AccessKey/SecretKey。
