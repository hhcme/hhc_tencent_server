...
- Dashboard 当前为 Phase 3 bootstrap：指标通过 SSH 即时采集，支持手动刷新和自动刷新；每次成功刷新会重新探测 Linux/systemd/SFTP 能力，写入 `server_capabilities` 独立缓存，并把完整快照写入 `dashboard_snapshots`，重新进入工作台时恢复最近快照；`/etc/os-release` 解析已覆盖 Ubuntu、Debian、CentOS 和 AlmaLinux 常见输出；已关联腾讯云 CVM 时会通过 Cloud Monitor `GetMonitorData` 拉取云侧 CPU、内存、磁盘读写和网络入出指标，已关联阿里云 ECS 时会通过 CMS `DescribeMetricList` 拉取同类云侧指标，已关联华为云 ECS 时会通过 CES `metric-data` 拉取同类云侧指标，并标记来源为 Cloud API；单项可选指标失败会以 warning 降级展示。更多业务层云监控指标仍待扩展。
