...
- SQLite repository：服务器 CRUD、trusted host key 级联删除、命令历史读写和按服务器清空、Dashboard 能力缓存和快照、操作日志、远程变更审计日志、云账号元数据、云实例关联、云盘、快照、计费状态、私有仓库实例和备份历史。
- Deployment persistence：部署项目、部署运行记录和部署日志的 SQLite 持久化、更新、按服务器/项目/run 查询和级联删除。
- GitLab Service persistence：按服务器持久化 GitLab CE 服务实例 external URL、package、版本、状态和 Web URL。
...
- Cloud provider foundation：adapter 协议、provider registry、capability 查询、统一错误、超时包装和共享并发节流。
...
- CloudResourceCenterSheet / CloudResourceCenterViewModel：按账号/地域同步云实例、云盘、快照和计费状态，支持跨资源搜索、类型/状态过滤、provider capability matrix 展示、capability Markdown 报告复制和资源详情查看。
- Cloud snapshot actions：腾讯云 CBS `CreateSnapshot` / `DeleteSnapshots`、阿里云 ECS `CreateSnapshot` / `DeleteSnapshot` 和华为云 EVS `cloudsnapshots` 创建/删除已接入云资源中心，磁盘可创建快照，腾讯云 `NORMAL` 快照、阿里云 `accomplished` 快照、华为云 `available` 快照可删除；所有操作需要风险确认，并写入 `remote_change_logs` 云端变更审计；提交成功后会尽力刷新快照列表，刷新失败时保留提交成功结果并提示。
- Cloud disk attachment actions：腾讯云 CBS `AttachDisks` / `DetachDisks`、阿里云 ECS `AttachDisk` / `DetachDisk` 和华为云 EVS `os-attach` / `os-detach` 已接入云资源中心；腾讯云 `UNATTACHED`/`DETACHED` 云盘、阿里云 `Available` 云盘、华为云 `available` 云盘可输入目标实例 ID 后挂载，腾讯云 `ATTACHED` 云盘、阿里云 `In_use` 云盘、华为云 `in-use` 云盘可卸载；所有操作需要风险确认，执行后本地缓存进入 `ATTACHING`/`DETACHING` 并尽力刷新云侧最终状态，同时写入 `remote_change_logs` 云端变更审计。
- Cloud power actions：腾讯云 CVM `StartInstances` / `StopInstances` / `RebootInstances`、阿里云 ECS `StartInstance` / `StopInstance` / `RebootInstance` 和华为云 ECS `os-start` / `os-stop` / `reboot` 已接入云资源中心；腾讯云 `STOPPED`、阿里云 `Stopped`、华为云 `SHUTOFF` 实例可启动，腾讯云 `RUNNING`、阿里云 `Running`、华为云 `ACTIVE` 实例可停止或重启；所有操作需要风险确认，执行后本地缓存进入 `STARTING`/`STOPPING`/`REBOOTING` 并尽力刷新实例状态，同时写入 `remote_change_logs` 云端变更审计。
...
- CronManager：通过 SSH/crontab 读取用户级 crontab，同时只读发现 `/etc/cron.d` 系统任务并解析 run-as user；用户级任务支持添加、启用、禁用和删除，写入前会把当前 crontab 备份到远端 `~/.hhc-crontab-backup-*`，系统任务在 UI 中以 read-only 展示。
...
- FirewallManager：通过 SSH 探测 firewalld、ufw、nftables、iptables 后端，读取后端状态和规则输出；支持受限规则写入，nftables 只会写入已有 `inet` / `ip` filter input/output chain，删除时只删除带 HHC comment 标记的规则；firewalld 安装但未运行时会展示 `not running`，不阻断其他功能。
...
- RemoteOperationRisk：为远程文件删除/权限修改、systemd、Cron、Nginx、Environment、Security Groups 和 Deployment 写操作生成统一风险级别、目标、命令预览、影响范围和恢复说明，供确认 UI 和审计流程复用。
- AddServerViewModel：表单校验。
- ServerWorkspaceViewModel：连接状态、主机指纹确认、smoke test、单条命令执行与取消、本次会话输出历史、stdout/stderr 分开展示、失败摘要、持久化命令元数据历史、历史命令重跑、Dashboard 手动/自动刷新、远程目录浏览、排队批量上传/下载、当前传输取消、待传队列暂停/恢复/排序/清空、传输任务状态记录、重命名、chmod 权限修改、可恢复移入回收目录、轻量文本编辑、腾讯云安全组查看和单条规则变更、systemd 服务管理、Cron 管理、Nginx 配置管理、Firewall 状态流、Environment 文件管理、Audit 日志查看和只含元数据的 Markdown 报告复制。
...
- Windows core tests：通过 GitHub Actions Windows runner 或 `scripts/ci-windows-core.ps1` 运行，覆盖 Windows Domain、SQLite repository、Credential Manager 平台边界、SQLite 凭据隔离、host key trust、OpenSSH `known_hosts` 粘贴和文件导入、SSH 状态机、密码/私钥添加服务器、编辑服务器 ViewModel、smoke test 编排、连接后单条命令执行、运行中命令取消、最近命令选择和 SQLite 命令历史元数据恢复。
...
部署集成测试复用同一组环境变量。它只会在远端 `/tmp/hhc-deploy-*` 下创建临时 source/bare/deploy 目录，完成 clone/fetch/checkout/build/health check 后尝试清理，不会触碰生产部署目录。2026-06-26 已用当前代码重新运行 `SSHIntegrationTests.testRealDeploymentRunnerDeploysTemporaryRepositoryWhenEnvironmentIsConfigured` 并通过。

GitLab Service 真实服务器验证默认跳过。只应在专用测试服务器上启用，preflight/status 可用：

```sh
export HHC_TEST_GITLAB_REAL="1"
```

真实安装会通过官方 Linux package 流程安装 `gitlab-ce`，要求空白或可重装的专用服务器，并且必须单独显式启用：

```sh
export HHC_TEST_GITLAB_INSTALL_REAL="1"
```

如果 `xcodebuild test` 启动的 GUI 测试宿主读不到一次性 shell 变量，可以把同名变量临时追加到本机 `~/.hhc_tencent_server_test_env`，运行完成后立即恢复文件。不要把该文件或真实值提交到仓库。
...
```sh
export HHC_TEST_SECURITY_REAL="1"
```

2026-06-26 已用当前代码重新运行 `SSHIntegrationTests.testRealSystemdAndCronWritesAuditAndCleanupWhenExplicitlyEnabled` 和 `SSHIntegrationTests.testRealEnvironmentFileSaveAuditsAndCleanupWhenExplicitlyEnabled` 并通过，覆盖临时 systemd oneshot unit、用户级 Cron add/disable/enable/delete、Environment 保存/备份、`remote_change_logs` 审计和结束后的远端残留检查。

Phase 4 Nginx 真实写入/reload 集成测试也默认跳过。它只会在目标服务器满足 `nginx -t` 通过、nginx 服务正在运行、存在受支持且可写的 include 配置目录时继续执行；测试会创建随机命名的临时 server block，保存后验证备份和审计，reload 后请求本机随机端口，最后删除临时配置并再次 reload：

```sh
export HHC_TEST_NGINX_REAL="1"
```

真实云账号只读同步集成测试默认跳过。它只执行 credential validate、地域/项目、实例、云盘、快照、计费状态和统一资源列表读取，不执行快照创建、云盘挂载、安全组写入或实例电源操作。推荐使用只读 CAM/RAM/IAM 子账号；凭据只放在本机环境变量或本机 env 文件，不提交到仓库：

```sh
export HHC_TEST_CLOUD_REAL="1"

export HHC_TEST_TENCENT_SECRET_ID="<secret id>"
export HHC_TEST_TENCENT_SECRET_KEY="<secret key>"
export HHC_TEST_TENCENT_REGION="ap-guangzhou"

export HHC_TEST_ALIBABA_ACCESS_KEY_ID="<access key id>"
export HHC_TEST_ALIBABA_ACCESS_KEY_SECRET="<access key secret>"
export HHC_TEST_ALIBABA_REGION="cn-hangzhou"

export HHC_TEST_HUAWEI_ACCESS_KEY_ID="<access key id>"
export HHC_TEST_HUAWEI_SECRET_ACCESS_KEY="<secret access key>"
export HHC_TEST_HUAWEI_REGION="cn-south-1"
```

也可以把同名变量放到本机专用文件，并通过 `HHC_TEST_CLOUD_ENV_FILE` 指定路径；未指定时测试会尝试读取 `~/.hhc-server-manager/cloud.env`：

```sh
export HHC_TEST_CLOUD_ENV_FILE="$HOME/.hhc-server-manager/cloud.env"
xcodebuild \
  -project HHCServerManager.xcodeproj \
  -scheme HHCServerManager \
  -configuration Debug \
  -destination 'platform=macOS' \
  CODE_SIGNING_ALLOWED=NO \
  test -only-testing:HHCServerManagerTests/CloudIntegrationTests
```
...
- 命令面板只持久化 command、exit code、duration 和 created at；stdout/stderr 默认只保留在本次工作台会话中，分开展示但不写入 SQLite，避免把敏感输出落盘。用户可按当前服务器清空已保存的命令历史元数据；清空成功会写入只包含删除条数的操作日志，不写入具体命令文本，已有操作日志不随之删除。
...
- 文件管理当前为 bootstrap：目录浏览通过 SSH `find` 命令实现，上传/下载优先通过本机 `rsync --partial --append-verify --progress` 走 OpenSSH 远程 shell，支持运行中解析字节、速度和 ETA 进度、保留部分文件并在恢复时追加校验；当 rsync 不可用或失败时会先回退到 OpenSSH `sftp -b`，首传使用普通 `put` / `get`，只有 partial 文件大于 0 且小于源文件大小时才使用 `put -a` / `get -a` 尝试续传，SFTP 仍失败时再回退到 `scp`。批量选择后的任务会进入受限并发队列，超过并发上限的任务保持 pending，并在 UI 中记录最近传输任务的 pending/running/succeeded/failed/cancelled/interrupted 状态；pending 队列可暂停/恢复，也可按真实调度顺序置为下一个、上移或下移，running 任务不会被打断。传输层提供 `RemoteFileTransferProgress` 回调，运行中任务可更新 `byteCount`、`progressFraction`、`transferRateBytesPerSecond`、`estimatedSecondsRemaining` 和状态消息并持久化到 `remote_file_transfers`，成功完成后展示 100%，重新进入工作台时恢复历史，遗留未完成任务会标记为 interrupted。失败、取消和中断任务可从历史中 Resume，恢复会复用原任务 ID、保留已有进度上下文，并在成功后覆盖同一条历史记录。当前运行中的传输可取消，待传队列可清空，终态传输历史可清理并写入只含删除数量的本地操作审计。重命名使用 `mv -n`，权限修改使用经过八进制校验的 `chmod`，删除入口会二次确认并移动到 `~/.hhc-server-manager-trash`；小型 UTF-8 文本文件可通过 SSH/base64 读取和保存，限制 256 KiB，保存前会生成 `*.hhc-backup-*` 备份，另存为默认不覆盖已有文件，并通过临时文件替换。已在真实 Linux 服务器上验证 `sftp` 命令存在、SFTP 上传/下载往返、SFTP partial upload/download 续传以及 scp 上传/下载往返可用；2026-06-26 已用当前代码重新运行 SSH smoke、SFTP 往返和 SFTP partial 续传 opt-in 集成测试并通过；尚未完成 SwiftNIO SSH/libssh2 正式 SFTP 替换和 native 级传输队列。
- Services 当前为 Phase 4 bootstrap：systemd 服务列表和日志通过 SSH 即时读取，start/stop/restart/reload 操作需要 UI 确认，unit 名限制为简单 `.service` 名称，并会写入 `remote_change_logs` 审计表；真实服务器已完成只读服务列表验证，并通过受控临时 oneshot unit 验证 restart 写操作、远端效果、审计日志和清理，2026-06-26 已用当前代码重新运行 `SSHIntegrationTests.testRealSystemdAndCronWritesAuditAndCleanupWhenExplicitlyEnabled` 并通过。
- 危险操作确认当前为 Phase 4+ 共享能力：`RemoteOperationRisk` 已为远程文件删除/权限修改、systemd、Cron、Nginx、Firewall、Environment、Security Groups 和 Deployment 生成统一风险级别、目标、命令预览、影响和恢复说明；现有确认弹窗已接入风险文案，chmod sheet 会展示风险预览，手动部署和回滚会在执行前展示部署命令预览、影响和审计提示。
- Cron 当前为 Phase 4 bootstrap：用户级 crontab 通过 SSH 即时读取，`/etc/cron.d` 系统任务会按文件来源和 run-as user 只读展示；添加/启用/禁用/删除仍限定在用户级 crontab，操作需要 UI 确认并在远端创建备份，同时会写入 `remote_change_logs` 审计表。真实服务器已完成只读 crontab 验证，并通过受控临时 cron entry 验证 add/disable/enable/delete 写操作、原 crontab 恢复、审计日志和清理，2026-06-26 已用当前代码重新运行 `SSHIntegrationTests.testRealSystemdAndCronWritesAuditAndCleanupWhenExplicitlyEnabled` 并通过；系统级 cron 当前仅支持只读发现，不直接写入 `/etc/cron.d`。
- Nginx 当前为 Phase 4 bootstrap：配置路径通过 `nginx -V` 动态探测，已覆盖 `/etc/nginx` 和 `/www/server/nginx/conf` 这类非标准安装路径；配置文件可浏览和编辑，保存时会先创建 `.hhc-backup-*` 远端备份，再写入配置并执行 `nginx -t`，测试失败会自动恢复备份；reload 需要 UI 确认并写入 `remote_change_logs` 审计表。真实服务器已完成 `nginx -t` 和配置目录只读验证，并已加入受保护的临时 server block 写入/reload 集成入口；当前测试服务器 nginx 服务未运行，真实配置写入/reload 仍需在合适环境继续验收。
- Firewall 当前为 Phase 4 bootstrap：探测 firewalld、ufw、nftables、iptables 并展示规则输出；支持受控新增/删除 IPv4 CIDR、TCP/UDP、单端口 allow/deny 规则，执行前展示风险确认和命令预览，成功或失败都会写入 `remote_change_logs` 并刷新规则快照；firewalld 当前仅支持 ingress，ufw/iptables 支持 ingress/egress，nftables 支持在已有 `inet` / `ip` filter input/output chain 上写入 HHC comment 标记规则，删除时按该标记查找 handle 后删除，不创建 table/chain 或修改默认 policy。真实服务器已验证 firewalld 安装但未运行的降级状态，真实规则写入仍需谨慎手动验收。
...
- Environment 当前为 Phase 4 bootstrap：只发现受限范围内的常见 env 文件，包括用户/应用目录 `.env` 和 `*.env`、`/etc/default`、`/etc/sysconfig`、`/etc/systemd/system/*.service.d/*.conf`；单文件限制 256 KiB，要求 UTF-8，保存前创建 `.hhc-backup-*` 远端备份并写入 `remote_change_logs`。真实服务器已通过临时 `$HOME/hhc-phase4-env-*` 目录验证 `.env` 发现、读取、保存、备份、审计日志和清理，2026-06-26 已用当前代码重新运行 `SSHIntegrationTests.testRealEnvironmentFileSaveAuditsAndCleanupWhenExplicitlyEnabled` 并通过。暂不自动解析 shell profile，不扫描私钥/凭据目录，不做跨文件变量合并。
- 云账号当前已实现本地元数据、云实例关联表、云实例安全组绑定 ID 持久化、云盘/快照/计费状态表、Keychain 云凭据命名空间、Tencent / Alibaba / Huawei Cloud adapter、云实例同步服务、腾讯云云盘/快照/计费状态同步服务、阿里云云盘/快照/安全组同步、快照创建/删除、云盘挂载/卸载、实例启动/停止/重启、安全组单条规则新增/删除和云侧 CPU/内存/磁盘/网络监控查询、华为云 EVS 云盘/快照/VPC 安全组同步、快照创建/删除、云盘挂载/卸载、实例启动/停止/重启、安全组单条规则创建/删除、实例/云盘计费状态和云侧 CPU/内存/磁盘/网络监控查询、腾讯云快照创建/删除操作、腾讯云云盘挂载/卸载操作、腾讯云实例启动/停止/重启操作、腾讯云安全组单条规则新增/删除、统一资源搜索、三家云基础导入 UI、云资源中心 UI、当前筛选资源摘要和 Markdown 报告复制、provider capability matrix Markdown 报告复制、provider-aware 云资源操作状态门禁、运行时权限失败 capability 降级、已关联腾讯云 CVM / 阿里云 ECS / 华为云 ECS 的云侧 Dashboard 指标查询和按关联实例过滤的安全组查询；三家云 adapter 的 HTTP 请求统一走超时和共享并发节流，默认最多 4 个云 API 请求同时进入 provider transport，等待中的请求取消时不会占住后续请求槽位；阿里云目前覆盖地域、ECS 实例、实例启动/停止/重启、云盘、云盘挂载/卸载、快照、安全组、安全组单条规则新增/删除、快照创建/删除、实例派生计费状态和 CMS Dashboard 指标，华为云目前覆盖项目发现、ECS 实例、实例启动/停止/重启、EVS 云盘、EVS 云盘挂载/卸载、EVS 快照、EVS 快照创建/删除、安全组、安全组单条规则创建/删除、实例/云盘派生计费状态和 CES Dashboard 指标，更多跨云危险云写操作仍在后续任务中。真实多云手动验收、真实腾讯云快照/云盘/实例电源/安全组写操作验收和更多跨云危险云写操作仍在后续任务中。
- Project Deployments 当前为 Phase 5 bootstrap：已建立 `deployment_projects`、`deployment_runs`、`deployment_logs` 三张表和 repository 读写基础，用于手动部署、日志、回滚和 webhook 核心触发；`DeploymentCommandBuilder` 已支持仓库 URL、branch、部署路径白名单和单行命令校验，并生成 clone/fetch/checkout/build/restart/health check 命令预览；`DeploymentRunner` 已能通过 `SSHClient` 执行受控步骤、记录脱敏后的 stdout/stderr/exit code、捕获 previous/target commit、失败停止并持久化 cancelled/failed/succeeded 状态；build 失败停止后续步骤、health check 失败、非法部署目录和工作台错误展示均有 mock/contract 测试覆盖；手动部署和 rollback 完成或失败后都会写入 `remote_change_logs`，记录 project target、动作、状态、摘要和 previous/target commit 快照；rollback 会通过受控 plan reset 到 previous commit，并重新执行 build/restart/health check，UI 已在执行前接入统一风险确认；webhook secret 存 Keychain，核心服务已支持 GitLab push payload 解析、`X-Gitlab-Token` 常量时间校验、repo/branch 过滤并触发 webhook run；本地 `DeploymentWebhookHTTPServer` 使用 Network.framework 监听 `/webhooks/gitlab`，按核心服务结果返回状态，并在 webhook run 开始和结束时写入 `operation_logs`；macOS workspace 已接入 Project Deployments 页面，支持项目列表、添加/编辑/删除、命令预览、手动运行、回滚、webhook 开关/secret、本地 listener 启停和 URL 展示、运行历史、运行中日志自动刷新、日志查看/复制，以及包含项目/run 元数据和脱敏日志表的 Markdown 运行报告复制，并明确客户端离线时 webhook 不会自动部署。真实服务器已完成临时 Git 仓库的等价手动部署 smoke，2026-06-26 已用当前代码重新运行部署 opt-in 集成测试并通过；生产项目部署仍需按具体项目配置单独验收。
- GitLab Service 当前为服务安装/管理 bootstrap：新增独立于 Project Deployments 的 GitLab CE 服务面板，默认使用官方 Linux package/Omnibus 路线安装 `gitlab-ce`，支持 external URL 校验、OS/包管理器/root 或 passwordless sudo/内存/磁盘/CPU/端口/已有 GitLab/curl/ca-certificates preflight，安装后读取 `gitlab-ctl status`、GitLab 版本、`/etc/gitlab/gitlab.rb` external URL、Web readiness 和最近日志；支持 start/stop/restart/reconfigure、浏览器打开 Web UI、只展示 `/etc/gitlab/initial_root_password` 路径提示而不读取保存初始密码，并为 install/service actions 写入 `remote_change_logs`，`targetType=gitlab_service`。v1 不安装 GitLab Runner，不做 Docker/HA/HTTPS 自动证书/恢复自动化；备份只展示 `gitlab-backup create` 命令预览。真实 GitLab 安装仍需专用服务器 opt-in 验收。
- Private Registries 当前为 Phase 6 bootstrap：已建立 Verdaccio 安装草稿、配置校验、固定版本策略、安装前 SSH preflight 检查、可读检查报告、基础 `config.yaml` 和 systemd service 模板；preflight 会检查 Node.js、npm/pnpm/yarn、systemd、htpasswd、端口占用、安装/data 目录父级可写性和磁盘空间，缺少 htpasswd 时以 warning 提示用户管理不可用但不阻断安装。`VerdaccioInstaller` 已生成受控远程安装流程，包含 system user、安装/data 目录创建、固定版本本地 npm install、配置写入、systemd unit 写入、`daemon-reload`、`enable --now`、`restart` 和带短重试的 `/-/ping` health check，并通过 mock/contract 测试覆盖失败停止和 health check 失败。`VerdaccioConfigurationBuilder` 已支持 `htpasswd` auth 配置、上游 registry URL policy、公开读/登录用户发布或登录用户读写两种包权限策略，以及独立 Nginx vhost 反向代理配置生成；proxy 生成包含 `proxy_pass`、Host/IP/Forwarded headers、Upgrade header 和 body size，HTTPS 仅保留配置协助注释，不自动申请证书。`NginxConfigManager` 已支持受控新建/更新 `.conf`，保存后执行 `nginx -t`，失败时恢复已有文件或删除新文件，reload 复用既有测试后 reload 流程。`VerdaccioManager` 已支持 systemd 状态、版本、storage size、journal tail 脱敏读取，start/stop/restart 服务控制，固定版本升级时备份 systemd unit 后写入新 unit、`daemon-reload`、重启和 health check，`config.yaml` UTF-8 读取和保存前 `.hhc-backup-*` 备份后重启，基于 storage 的私有包摘要列表，基于 `htpasswd -B -i` / `htpasswd -D` 的用户创建、改密和删除命令层，包含 `config.yaml` 和 storage 目录的 tar.gz 备份归档，受限备份路径恢复流程，以及 npm smoke test 流程；smoke test 会创建临时 scoped package，通过临时 `.npmrc` 写入运行期生成的 `_auth` 后执行 `npm publish`、`npm install` 和 `require` 验证，并在退出时尝试 `npm unpublish` 清理，明文密码不会进入 shell 命令字符串。恢复会先停止服务并创建 rollback 归档，恢复后执行 health check，恢复命令或 health check 失败时会尝试回滚。SQLite 已接入 `registry_instances` 和 `registry_backups`，在调用方传入 repository 时记录备份/恢复成功或失败历史、大小、恢复时间和脱敏错误信息；服务控制、固定版本升级、备份/恢复和 htpasswd 用户创建/改密/删除都会写入 `remote_change_logs`。macOS 工作台已新增 Registries section，当前提供 Verdaccio 安装参数编辑、preflight、安装确认、状态、带统一风险确认的服务 start/stop/restart、带统一风险确认的固定版本 upgrade、htpasswd 用户创建/改密/确认删除、npm publish/install smoke test、包列表、备份创建/恢复入口、Nginx proxy vhost 写入/测试结果展示/确认 reload，以及 Dart/Flutter Hosted Pub Repository 配置助手；安装参数可编辑名称、固定版本、安装路径、数据路径、监听地址/端口和 service 名，并会在 preflight / install 前拦截非法草稿，避免触发远端命令；创建备份后会回填恢复路径，恢复前需要危险确认，恢复后会刷新状态。Dart/Flutter pub 方案已完成当前轮技术结论：Phase 6 不实现自托管 pub registry installer，改为提供外部 Hosted Pub Repository 的 URL/package/env var 校验、`pubspec.yaml` dependency、`publish_to`、token 命令和 publish/get 命令生成；unpub / pub_server 安装能力必须等真实 Dart/Flutter 项目 publish/get 验收后再开放。真实测试服务器已完成隔离 Verdaccio 安装、htpasswd 用户、npm publish/install/require、服务重启、配置修改前备份、tar.gz 备份和恢复验收；真实 Nginx proxy 写入/reload 仍需谨慎手动验收。
