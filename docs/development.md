...
- AlibabaCloudAdapter：阿里云 OpenAPI V3 `ACS3-HMAC-SHA256` 签名、ECS `DescribeRegions` / `DescribeInstances` / `DescribeDisks` / `DescribeSnapshots` / `DescribeSecurityGroups` / `DescribeSecurityGroupAttribute` 只读查询、分页、公网/私网 IP、规格、状态、VPC、云盘类型/容量/挂载实例、快照来源云盘/大小/创建时间、安全组和规则、计费和到期字段映射、mock transport fixture 测试。
...
- 云账号当前已实现本地元数据、云实例关联表、云盘/快照/计费状态表、Keychain 云凭据命名空间、Tencent / Alibaba / Huawei Cloud adapter、云实例同步服务、腾讯云云盘/快照/计费状态同步服务、阿里云云盘/快照/安全组只读同步、华为云 EVS 云盘/快照和 VPC 安全组只读同步、腾讯云快照创建/删除操作、腾讯云云盘挂载/卸载操作、腾讯云实例启动/停止/重启操作、腾讯云安全组单条规则新增/删除、统一资源搜索、三家云基础导入 UI、云资源中心 UI、已关联 CVM 的 CPU 云监控查询和账号地域级安全组查询；阿里云目前覆盖地域、ECS 实例、云盘、快照、安全组和实例派生计费状态，华为云目前覆盖项目发现、ECS 实例、EVS 云盘、EVS 快照、安全组和实例派生计费状态，更多跨云危险云写操作仍在后续任务中。真实多云手动验收、真实腾讯云快照/云盘/实例电源/安全组写操作验收和更多跨云危险云写操作仍在后续任务中。
