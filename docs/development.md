...
- AlibabaCloudAdapter：阿里云 OpenAPI V3 `ACS3-HMAC-SHA256` 签名、ECS `DescribeRegions` / `DescribeInstances` / `DescribeDisks` / `DescribeSnapshots` / `CreateSnapshot` / `DeleteSnapshot` / `AttachDisk` / `DetachDisk` / `DescribeSecurityGroups` / `DescribeSecurityGroupAttribute` 查询与操作、分页、公网/私网 IP、规格、状态、VPC、云盘类型/容量/挂载实例、快照来源云盘/大小/创建时间、安全组和规则、计费和到期字段映射、mock transport fixture 测试。
- HuaweiCloudAdapter：华为云 AK/SK `SDK-HMAC-SHA256` 签名、IAM project 发现、ECS `cloudservers/detail`、EVS `cloudvolumes/detail`、EVS `snapshots/detail`、EVS `cloudsnapshots` 创建/删除、VPC `security-groups` 和 VPC `security-group-rules` 只读查询、分页、公网/私网 IP、规格、可用区、状态、云盘类型/容量/挂载实例、快照来源云盘/大小/创建时间、安全组和规则、计费元数据映射、mock transport fixture 测试。
...
- Cloud disk attachment actions：腾讯云 CBS `AttachDisks` / `DetachDisks` 和阿里云 ECS `AttachDisk` / `DetachDisk` 已接入云资源中心；腾讯云 `UNATTACHED`/`DETACHED` 云盘、阿里云 `Available` 云盘可输入目标实例 ID 后挂载，腾讯云 `ATTACHED` 云盘、阿里云 `In_use` 云盘可卸载；所有操作需要风险确认，执行后本地缓存进入 `ATTACHING`/`DETACHING`，并写入 `remote_change_logs` 云端变更审计。
...
- 文件管理当前为 bootstrap：目录浏览通过 SSH `find` 命令实现，上传/下载通过本机 OpenSSH `scp` 实现批量选择后的串行排队传输，并在 UI 中记录最近传输任务的 pending/running/succeeded/failed/cancelled 状态；运行中任务展示不确定进度，成功完成后展示 100%，完成/失败/取消历史会写入 `remote_file_transfers` 并在重新进入工作台时恢复；当前运行中的传输可取消，待传队列可清空。重命名使用 `mv -n`，权限修改使用经过八进制校验的 `chmod`，删除入口会二次确认并移动到 `~/.hhc-server-manager-trash`；小型 UTF-8 文本文件可通过 SSH/base64 读取和保存，限制 256 KiB，保存前会生成 `*.hhc-backup-*` 备份，另存为默认不覆盖已有文件，并通过临时文件替换。已在真实 Linux 服务器上验证 `sftp` 命令存在以及 scp 上传/下载往返可用；尚未完成 SwiftNIO SSH/libssh2 正式 SFTP 替换、字节级实时进度、并发传输和可恢复队列持久化。
...
- Security Groups 当前为 Phase 4 bootstrap：仅对已关联云实例的账号和地域启用，使用腾讯云 VPC `DescribeSecurityGroups` / `DescribeSecurityGroupPolicies` 查询并展示安全组规则，阿里云使用 ECS `DescribeSecurityGroups` / `DescribeSecurityGroupAttribute` 提供安全组和规则只读查看，华为云使用 VPC `security-groups` / `security-group-rules` 提供安全组和规则只读查看；当前支持本地生成安全组规则新增/删除 diff、before/after 计数、风险级别、命令预览和警告，并可通过腾讯云 VPC `AuthorizeSecurityGroupIngress` / `AuthorizeSecurityGroupEgress` / `RevokeSecurityGroupIngress` / `RevokeSecurityGroupEgress` 执行经过确认的单条规则变更。成功或失败都会写入 `remote_change_logs` 并刷新规则快照。当前尚未持久化实例与安全组的精确关联，因此先展示该账号地域下的安全组列表，后续补实例过滤和多云写操作适配。
...
- 云账号当前已实现本地元数据、云实例关联表、云盘/快照/计费状态表、Keychain 云凭据命名空间、Tencent / Alibaba / Huawei Cloud adapter、云实例同步服务、腾讯云云盘/快照/计费状态同步服务、阿里云云盘/快照/安全组同步、快照创建/删除和云盘挂载/卸载操作、华为云 EVS 云盘/快照/VPC 安全组同步和快照创建/删除操作、腾讯云快照创建/删除操作、腾讯云云盘挂载/卸载操作、腾讯云实例启动/停止/重启操作、腾讯云安全组单条规则新增/删除、统一资源搜索、三家云基础导入 UI、云资源中心 UI、已关联 CVM 的 CPU 云监控查询和账号地域级安全组查询；阿里云目前覆盖地域、ECS 实例、云盘、云盘挂载/卸载、快照、安全组、快照创建/删除和实例派生计费状态，华为云目前覆盖项目发现、ECS 实例、EVS 云盘、EVS 快照、EVS 快照创建/删除、安全组和实例派生计费状态，更多跨云危险云写操作仍在后续任务中。真实多云手动验收、真实腾讯云快照/云盘/实例电源/安全组写操作验收和更多跨云危险云写操作仍在后续任务中。
...
- AlibabaCloudAdapter 已接入阿里云 OpenAPI V3 header 签名流程，并实现 ECS `DescribeRegions`、`DescribeInstances`、`DescribeDisks`、`DescribeSnapshots`、`CreateSnapshot`、`DeleteSnapshot`、`AttachDisk`、`DetachDisk`、`DescribeSecurityGroups`、`DescribeSecurityGroupAttribute` 查询与操作；HuaweiCloudAdapter 已接入华为云 AK/SK 签名流程，并实现 IAM project 发现、ECS `cloudservers/detail`、EVS `cloudvolumes/detail`、EVS `snapshots/detail`、EVS `cloudsnapshots` 创建/删除、VPC `security-groups` 和 VPC `security-group-rules` 只读查询。默认测试使用 mock transport，不提交真实 AccessKey/SecretKey。
