...
- SSHIntegrationTests：通过环境变量启用，默认跳过；包含真实 SSH smoke test、SFTP 往返、临时 Git 仓库部署闭环、Phase 4 systemd/Cron/Environment 受控写操作和 Verdaccio lifecycle 集成用例。
...
- Cron 当前为 Phase 4 bootstrap：用户级 crontab 通过 SSH 即时读取，添加/启用/禁用/删除操作需要 UI 确认并在远端创建备份，同时会写入 `remote_change_logs` 审计表；真实服务器已完成只读 crontab 验证，并通过受控临时 cron entry 验证 add/disable/enable/delete 写操作、原 crontab 恢复和审计日志。尚未支持系统级 `/etc/cron*` 管理。
