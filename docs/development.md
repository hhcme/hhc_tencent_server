...
- HuaweiCloudAdapter：华为云 AK/SK `SDK-HMAC-SHA256` 签名、IAM project 发现、ECS `cloudservers/detail`、ECS `os-start` / `os-stop` / `reboot` 实例启动/停止/重启、EVS `cloudvolumes/detail`、EVS `snapshots/detail`、EVS `cloudsnapshots` 创建/删除、EVS `os-attach` / `os-detach` 云盘挂载/卸载、VPC `security-groups` / `security-group-rules` 查询、VPC v3 `security-group-rules` 单条规则创建/删除、分页、公网/私网 IP、规格、可用区、状态、云盘类型/容量/挂载实例、快照来源云盘/大小/创建时间、安全组和规则、计费元数据映射、mock transport fixture 测试。
...
- Cloud power actions：腾讯云 CVM `StartInstances` / `StopInstances` / `RebootInstances` 和阿里云 ECS `StartInstance` / `StopInstance` / `RebootInstance` 已接入云资源中心，腾讯云 `STOPPED`、阿里云 `Stopped` 实例可启动，腾讯云 `RUNNING`、阿里云 `Running` 实例可停止或重启；所有操作需要风险确认，执行后本地缓存进入 `STARTING`/`STOPPING`/`REBOOTING`，并写入 `remote_change_logs` 云端变更审计。
...
- Dashboard 当前为 Phase 3 bootstrap：指标通过 SSH 即时采集，支持手动刷新和自动刷新；已关联腾讯云 CVM 时会通过 Cloud Monitor `GetMonitorData` 拉取 Cloud CPU 指标，已关联阿里云 ECS 时会通过 CMS `DescribeMetricList` 拉取 Cloud CPU 指标，并标记来源为 Cloud API；单项可选指标失败会以 warning 降级展示；成功刷新会写入 `dashboard_snapshots` 缓存表并在重新进入工作台时恢复最近快照。更多云监控指标仍待扩展。
...
- 云账号当前已实现本地元数据、云实例关联表、云盘/快照/计费状态表、Keychain 云凭据命名空间、Tencent / Alibaba / Huawei Cloud adapter、云实例同步服务、腾讯云云盘/快照/计费状态同步服务、阿里云云盘/快照/安全组同步、快照创建/删除、云盘挂载/卸载、实例启动/停止/重启、安全组单条规则新增/删除和 CPU 云监控查询、华为云 EVS 云盘/快照/VPC 安全组同步、快照创建/删除、云盘挂载/卸载、实例启动/停止/重启和安全组单条规则创建/删除操作、腾讯云快照创建/删除操作、腾讯云云盘挂载/卸载操作、腾讯云实例启动/停止/重启操作、腾讯云安全组单条规则新增/删除、统一资源搜索、三家云基础导入 UI、云资源中心 UI、已关联腾讯云 CVM / 阿里云 ECS 的 CPU 云监控查询和账号地域级安全组查询；阿里云目前覆盖地域、ECS 实例、实例启动/停止/重启、云盘、云盘挂载/卸载、快照、安全组、安全组单条规则新增/删除、快照创建/删除、实例派生计费状态和 CMS CPU 指标，华为云目前覆盖项目发现、ECS 实例、实例启动/停止/重启、EVS 云盘、EVS 云盘挂载/卸载、EVS 快照、EVS 快照创建/删除、安全组、安全组单条规则创建/删除和实例派生计费状态，更多跨云危险云写操作仍在后续任务中。真实多云手动验收、真实腾讯云快照/云盘/实例电源/安全组写操作验收和更多跨云危险云写操作仍在后续任务中。
