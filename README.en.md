...
The repository is in early macOS implementation. The app can already save server profiles, store SSH and cloud credentials in Keychain, verify SSH host keys, run a real OpenSSH smoke test, execute single remote commands, browse remote directories, and persist command/cloud metadata in SQLite.
...
- Remote file browser foundations with path navigation, directory listing, file metadata display, rename, recoverable move-to-trash, and lightweight UTF-8 text editing with backup-on-save.
...
3. **Phase 3: Dashboard and file manager**
   Combine SSH metrics with cloud metrics; ship SSH-bootstrap directory browsing and lightweight editing first, then validate SFTP for upload, download, and transfer queues.
