...
- Project Deployments with GitLab repository project CRUD, command previews, logs, rollback, local webhook listener, and live run/log refresh.
- GitLab Service basics: install GitLab CE through the official Linux package flow, run preflight checks, show version, status, external URL, recent logs, initial root password path hint, and open the Web UI in a browser.
...
5. **Phase 5: Project Deployments**
   Manual deployment, deployment logs, rollback, and webhook automation for GitLab repository projects.
...
The macOS app is now under active implementation. Phase 1 through Phase 6 foundations are in place across SSH, optional cloud APIs, dashboard, file management, security/environment tools, deployments, and private registry management: SwiftUI app structure, local SQLite persistence, Keychain-backed SSH/cloud credentials, host-key trust, OpenSSH-based real command execution with cancellation, command metadata history with reruns, split stdout/stderr output, cloud account metadata, cloud instance links, Tencent/Alibaba/Huawei adapters, linked-cloud dashboard metrics, remote directory browsing and editing, limited-concurrency rsync/sftp/scp batch transfers with byte/speed/ETA progress, rsync `--append-verify` resume, SFTP `put -a` / `get -a` fallback, and retry for interrupted history, systemd/Cron/Nginx/Firewall/Environment workflows, read-only `/etc/cron.d` discovery, remote change audit logs, Project Deployments runs/logs/rollback/webhook listener/redacted run reports, GitLab CE Linux package preflight/install/status/service control/browser entry, Verdaccio preflight/install/service/users/packages/backup/restore/Nginx proxy/npm smoke flows, Cloud Resources search/action summaries/provider capability reports/Markdown reports, unit tests, CI, and real-server smoke validation for SSH, transfers, Phase 4 systemd/Cron/Environment controlled writes, temporary deployment, and isolated Verdaccio install/publish/restart/backup/restore. Windows native Phase 8 has started with a WinUI 3 / Windows App SDK / .NET solution skeleton, CI-covered core layers, SQLite command history metadata, and an opt-in Windows-only real SSH/Credential Manager integration test. Command output remains session-scoped and is not persisted by default. SwiftNIO/libssh2 SFTP hardening, native transfer queue, GitLab real install validation, production deployment validation, real multi-cloud write-operation validation, production Verdaccio/proxy validation, and full Windows-host validation remain planned work.
