...
- Added Windows core OpenSSH `known_hosts` import support with plain host parsing, non-default port matching, SHA256 fingerprint derivation, and SQLite trust-store tests.
- Added remote change audit logs for macOS remote file move-to-trash and chmod actions, including before/after snapshots and ViewModel coverage.
...
- Added Dashboard no-`/proc` degradation coverage with warning preservation and `NAME` + `VERSION_ID` OS fallback.
...
- Added a real SSH deployment integration smoke for temporary Git repositories with injectable deployment path policy.
...
- Added remote change audit logs for Verdaccio backup, restore, and htpasswd user create/update/delete actions.
- Hardened Verdaccio runtime installation to use a locally installed pinned package instead of runtime `npx`, added retrying health checks, switched npm smoke auth to temporary `.npmrc`, and added an opt-in real SSH lifecycle test for isolated install, publish/install, restart, config backup, backup, and restore.
...
- 添加华为云云监控 `metric-data` 支持，可为已关联 ECS 展示 Cloud CPU 指标。
- 添加华为云实例和 EVS 云盘计费状态同步，来源于 ECS/EVS 元数据。
...
- 添加真实 SSH 临时 Git 仓库部署集成验收，并支持 DeploymentRunner 注入部署路径策略。
