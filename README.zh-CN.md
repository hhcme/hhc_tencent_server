...
- 远程文件浏览基础：支持路径导航、目录列表、文件元信息展示、基于 OpenSSH/rsync/sftp/scp 的有限并发批量上传/下载、运行中字节/速度/ETA 进度、rsync append 校验续传、SFTP `put -a` / `get -a` 回退续传、当前传输取消、待传队列暂停/恢复/排序/清空、带审计的终态传输历史清理、失败/取消/中断任务原地恢复、重命名、基于 chmod 的权限修改、可恢复移入回收目录，以及带保存前备份和另存为的轻量 UTF-8 文本编辑。
...
- Audit 工作台：查看当前服务器远程变更日志和本地操作日志，并可复制只含元数据的 Markdown 报告。
