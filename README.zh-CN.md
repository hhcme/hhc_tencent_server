...
- 远程文件浏览基础：支持路径导航、目录列表、文件元信息展示、基于 OpenSSH/scp 的单文件上传/下载、重命名、可恢复移入回收目录，以及带保存前备份的轻量 UTF-8 文本编辑。
