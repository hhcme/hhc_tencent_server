...
当前仓库处于 macOS 早期实现阶段。应用已经可以保存服务器配置、将 SSH 和云凭据存入 Keychain、校验 SSH 主机指纹、执行真实 OpenSSH smoke test、执行单条远程命令、浏览远程目录，并在 SQLite 中持久化命令和云资源元数据。
...
- 远程文件浏览基础：支持路径导航、目录列表、文件元信息展示、重命名、可恢复移入回收目录，以及带保存前备份的轻量 UTF-8 文本编辑。
...
3. **Phase 3：Dashboard + 文件管理器**
   聚合 SSH 指标和云监控指标；先交付基于 SSH bootstrap 的目录浏览和轻量编辑，再验证 SFTP 以支持上传、下载和传输队列。
4. **Phase 4：安全组 + 环境配置**
